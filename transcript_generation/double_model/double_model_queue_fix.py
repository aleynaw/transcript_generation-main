...
def chunk_questions(sections):
    ...
    for section in sections:
        ...
        for sub_section in section[1:]:
            ...
            sub_section_chunks = []  # Start with combined title
            
            i = 1  # Start from the first question in the sub-section
            while i < len(sub_section):
                question = sub_section[i]
                
                # If the question starts with "-", start a new combined question
                if question.startswith("-"):
                    combined_question = question  # Start with the main question
                    
                    # Check only the next question for "+"
                    if i + 1 < len(sub_section) and sub_section[i + 1].startswith("+"):
                        combined_question += "\n" + sub_section[i + 1]  # Concatenate with newline
                        i += 1  # Skip the next question, as it has been added to combined_question
                    
                    # Add the combined question to sub-section chunks
                    sub_section_chunks.append(combined_question)
...
def chat_between_models(message_list, asst_prompt, patient_prompt, starter):
    ...
    just_questions = [question for sublist in chunks for question in sublist]
    # print("Just Questions: ", just_questions)
    # print("Questions: ", just_questions)
    ...
    first_question = questions_queue.popleft()
    curr_question = first_question
    questions_list = [curr_question]
    ...
    while not stop_condition and turns < max_turns:
        ...
        if questions_queue: 
            ...
            print("Next question to add: ", questions_queue[0])
            next_question = questions_queue.popleft()
            
            if turns > initial_counter:
                print()
                print("Dumping first questions.........................................................")
                i=0
                
                print("Dumping ", questions_list[0])
            ...
            questions_list.append(curr_question)
            print("Questions List: ", questions_list)
